...
class BboxLoss(nn.Module):
    ...
    Attributes:
        reg_max (int): Regularization maximum for DFL.
        dfl_loss (DFLoss | None): DFL loss module, None when reg_max=1.
        box_loss_type (str): IoU loss type - 'ciou' (default), 'shape_iou', 'diou', 'giou'.
        shape_iou_scale (float): Scale factor for Shape-IoU (only used when box_loss_type='shape_iou').
    """

    def __init__(self, reg_max: int = 16, box_loss_type: str = "ciou", shape_iou_scale: float = 0.0):
        """Initialize the BboxLoss module with regularization maximum and DFL settings.

        Args:
            reg_max (int): Regularization maximum for DFL.
            box_loss_type (str): IoU loss type - 'ciou', 'shape_iou', 'diou', 'giou'.
            shape_iou_scale (float): Scale factor for Shape-IoU.
        """
        super().__init__()
    ...
    def forward(
        ...
        iou = self._compute_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask])
    ...
    def _compute_iou(self, pred_bboxes: torch.Tensor, target_bboxes: torch.Tensor) -> torch.Tensor:
        """Compute IoU between predicted and target bounding boxes based on box_loss_type."""
        if self.box_loss_type == "shape_iou":
            return shape_iou(pred_bboxes, target_bboxes, xywh=False, scale=self.shape_iou_scale)
        elif self.box_loss_type == "ciou":
            return bbox_iou(pred_bboxes, target_bboxes, xywh=False, CIoU=True)
        elif self.box_loss_type == "diou":
            return bbox_iou(pred_bboxes, target_bboxes, xywh=False, DIoU=True)
        elif self.box_loss_type == "giou":
            return bbox_iou(pred_bboxes, target_bboxes, xywh=False, GIoU=True)
        else:
            return bbox_iou(pred_bboxes, target_bboxes, xywh=False, CIoU=True)
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        # Box loss type: 'ciou' (default), 'shape_iou', 'diou', 'giou'
        self.box_loss_type = getattr(h, "box_loss_type", "ciou")
        self.shape_iou_scale = getattr(h, "shape_iou_scale", 0.0)
